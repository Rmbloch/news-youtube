...
def create_video(audio_files, image_files, output_file):
    if not os.path.exists("minecraft.mp4"):
        yt = YouTube("https://www.youtube.com/watch?v=u7kdVe8q5zs")
    ...
    for i in range(len(audio_files)):
        ...
        image = ImageClip(image_files[i]).resize((1920, 1080)).set_duration(audio.duration)
        clips.append(image)
    ...
    image_resize = image_video.resize(0.5)
